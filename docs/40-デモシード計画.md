...
> **この節の手順は `scripts/doDemoSeedEdit.sh` に包んだ (docs/48)。**
> 手元から `./doDemoSeedEdit.sh start` → ブラウザで編集 →
> `./doDemoSeedEdit.sh commit` で、下の createdb -T と検証がそのまま走る。
> 以下はその中身の説明として残す (罠の理解用)。
...
- ~~**種作成の自動化スクリプト**~~ — `scripts/doDemoSeedEdit.sh` に
  起こした (docs/48)。§3 の手順を編集セッション (start/commit/abort/status)
  として包んである
