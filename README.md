...
⚠️ **`compose.yaml` は転送されない。** 送るのはイメージだけで、
`compose.yaml` と `.env` はサーバに配置済みであることが前提。
**`compose.yaml` を変えたら手で送ること**:

```bash
scp compose.yaml vps2:41-QR-search/qr-search/compose.yaml
ssh vps2 "cd 41-QR-search/qr-search && docker compose up -d app"
```
